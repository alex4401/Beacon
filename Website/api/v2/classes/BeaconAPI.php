...
abstract class BeaconAPI {
	...
	private static function AuthorizeWithPassword(string $username, string $password) {
		...
		if (is_null($user) == false && $user->IsEnabled() && $user->TestPassword($password, $upgrade)) {
	...
	public static function URL(string $path = '/') {
		...
		$domain = BeaconCommon::InProduction() ? 'api.usebeacon.app' : BeaconCommon::EnvironmentName() . '-api.usebeacon.app';
